// Dark Mode Script

let darkMode = localStorage.getItem('darkMode'); 
const darkModeToggle = document.querySelector('#dark-mode-toggle');

const enableDarkMode = () => {
  // 1. Add the class to the body
  document.body.classList.add('darkmode');
  // 2. Update darkMode in localStorage
  localStorage.setItem('darkMode', 'enabled');
}

const disableDarkMode = () => {
  // 1. Remove the class from the body
  document.body.classList.remove('darkmode');
  // 2. Update darkMode in localStorage 
  localStorage.setItem('darkMode', null);
}

// If the user already visited and enabled darkMode
// start things off with it on
if (darkMode === 'enabled') {
  enableDarkMode();
}

// When someone clicks the button
darkModeToggle.addEventListener('click', () => {
  // get their darkMode setting
  darkMode = localStorage.getItem('darkMode'); 
  
  // if it not current enabled, enable it
  if (darkMode !== 'enabled') {
    enableDarkMode();
  // if it has been enabled, turn it off  
  } else {  
    disableDarkMode(); 
  }
});

// White Mode Script

let whiteMode = localStorage.getItem('whiteMode'); 
const whiteModeToggle = document.querySelector('#white-mode-toggle');

const enableWhiteMode = () => {
  // 1. Add the class to the body
  document.body.classList.add('whitemode');
  // 2. Update whiteMode in localStorage
  localStorage.setItem('whiteMode', 'enabled');
}

const disableWhiteMode = () => {
  // 1. Remove the class from the body
  document.body.classList.remove('whitemode');
  // 2. Update whiteMode in localStorage 
  localStorage.setItem('whiteMode', null);
}

// If the user already visited and enabled whiteMode
// start things off with it on
if (whiteMode === 'enabled') {
  enableWhiteMode();
}

// When someone clicks the button
whiteModeToggle.addEventListener('click', () => {
  // get their whiteMode setting
  whiteMode = localStorage.getItem('whiteMode'); 
  
  // if it not current enabled, enable it
  if (whiteMode !== 'enabled') {
    enableWhiteMode();
  // if it has been enabled, turn it off  
  } else {  
    disableWhiteMode(); 
  }
});